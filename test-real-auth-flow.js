...
console.log('🔍 Testando fluxo de autenticação real...');
...
async function testRealAuthFlow() {
  try {
    console.log('🔐 Fazendo login com credenciais reais...');
    
    // Usar credenciais de admin conhecidas (substitua por credenciais válidas)
    const email = 'fontedevidalaranjeiras@gmail.com'; // Email principal do admin
    const password = 'sua_senha_aqui'; // Você precisa fornecer a senha real
    ...
    const testDoc = {
      title: 'Teste Real Auth Flow',
      content: 'Documento criado durante teste de autenticação real',
...
testRealAuthFlow().then(() => {
  console.log('✅ Teste do fluxo de autenticação real concluído');
  process.exit(0);
}).catch((error) => {
